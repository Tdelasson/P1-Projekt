...
// Open the resident record file in read mode
    ...
    printf("Medication for adam: %s", medications[0].medication[0]);
    ...
    /*for (int g = 0; g < MAX_PATIENTS; g++) {
        ...
        for (int j = 0; j < MAX_MEDICATIONS; j++){
            ...
            printf("Medication: %s \n Medication unit: %s \n Total daily dose: %d \n "
                   "Morning dose: %d \n Noon dose: %d Evening dose: %d  \n\n",
                   medications[g].medication[j], medications[g].medication_unit[j],
                   medications[g].total_daily_dose[j], medications[g].morning_dose[j],
                   medications[g].noon_dose[j], medications[g].evening_dose[j]);
        }
    }*/
